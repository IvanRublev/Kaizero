...
**`KAIZERO_WATCHDOG`** — how long one `claude` may make no progress before it is killed. Default `15m`; accepts plain seconds or an `s`/`m`/`h` suffix (`900`, `90s`, `15m`, `1h`), and `0` disables it. A `claude` that stops making progress never exits, so without the timer the loop parks on it forever — no restart, no report, and nothing left to stop but the whole run. Progress is measured as `claude`'s own cumulative CPU time, not wall clock: one that is thinking, streaming, or running tools burns CPU and keeps resetting the window however long the Task takes, while one blocked on a dead socket burns none. The countdown never starts before `claude` has written its first record — a slow startup (a large system prompt, subagent setup) is read as "just getting going," not "already stalled," and the full budget still applies once that first record lands. When the timer fires it says so on its own console line, then `SIGTERM`s `claude` (the same signal the Stop hook uses, so the restart path is the usual one) and escalates to `SIGKILL` 10s later. The Task worktree survives the kill: the next session reclaims it through the crash-recovery path.

```sh
KAIZERO_WATCHDOG=45m kaizero todo.md
```

**`KAIZERO_QUOTA_RETRY`** — gap between retries once `claude` hits its own usage limit (a `quotaLimits`-rejected turn). Default `15m`; same grammar as `KAIZERO_WATCHDOG` (`900`, `90s`, `15m`, `1h`). Restarting immediately after a quota rejection is certain to be rejected again, so kaizero prints the API's own message verbatim (it already names the reset time — nothing is recomputed) and holds the restart, retrying at this interval — capped so the last retry lands exactly at the reported reset time — rather than relaunching every ordinary `RESTART_WAIT` tick. The wait is interruptible (`Ctrl+C`/`SIGTERM` stop it the same as any other pause) and never outlasts the claimed Task's worktree/lock, which the instance keeps for the whole wait. A missing, unparseable, or already-past reset time falls back to the ordinary immediate restart.
