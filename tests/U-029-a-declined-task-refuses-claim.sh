...
# U-029-a-declined-task-refuses-claim — a `[⛔]` task refused by `claim`.
# Needs real claude: no — a stub `claude` on a scenario-scoped PATH stands in for it
# Tools beyond the shared prerequisites: none
# Folder under $TESTROOT: $TESTROOT/U-029-a-declined-task-refuses-claim
# Wall-clock budget: its longest Run command is `timeout 20` — allow that command at least 20s
#
# `claim` refuses a `[⛔]` box as landed exactly like any other landed box.

### Setup
TU="$TESTROOT/U-029-a-declined-task-refuses-claim"; mkdir -p "$TU/bin" "$TU/stub"
...
### U43 — `zero.sh claim` refuses a `[⛔]` task as already landed, same as any other landed box
mrsetup u43
( cd "$TU/u43"
  printf '# todo\n\n- [\xe2\x9b\x94] u43a declined already\n' > todo.md
  mkdir -p tasks; printf -- '### Acceptance criteria\n- [ ] x\n' > tasks/u43a.md
  git add -A; git commit -qm "u43a declined"
)
# u43drive.sh runs as its own subprocess so it gets its own real $$ to own the session record
# with (own_session's default target); check() is exported by test-setup.sh so it's usable there
# too — that subprocess's own FAILED/ERRORED stay local to it, folded into this script's own
# FAILED below via its exit status.
cat > "$TU/u43drive.sh" <<'DRIVE'
set -uo pipefail
FAILED=0; ERRORED=0   # this subprocess's own check() state — never inherited, see test-setup.sh
own_session "$TU/u43.rec"
cd "$TU/u43"
out=$(bash .git/zero.sh claim u43a 2>&1); rc=$?
check "U43 claim on a [⛔] task refuses" "$([ "$rc" != 0 ] && echo nonzero || echo 0)" "nonzero"
check "U43 refusal names it already landed" "$(printf '%s' "$out" | grep -c 'already landed')" "1"
[ "$FAILED" = 0 ] && [ "$ERRORED" = 0 ]
DRIVE
export TU
bash "$TU/u43drive.sh" || FAILED=1
# U43 PASS — is_done/box_checked_on_base treat `[⛔]` as landed exactly like `[x]`/`[↑]`/`[?]`, so
# `claim` refuses it before any worktree work; combined with T22 (all_todos_done counts it done)
# and U18 (the MR prompt's legend names it a blocking prerequisite, never claimable), a `[⛔]`
# task is unreachable from every angle a session or the shell could reach it from.
