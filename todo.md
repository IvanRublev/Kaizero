...
- [ ] BUG-068 `merge_same_repo`'s already-landed detection is an ancestry check that is trivially true the moment it runs, so it cannot tell "code already landed" apart from "nothing was ever committed" tasks/BUG-068.md
